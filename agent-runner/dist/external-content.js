...
const INSTRUCTION_LIKE_FRAGMENTS = [
...
export function sanitizeExternalContentText(content) {
    let sanitized = content.replace(/\r\n?/g, '\n');
    for (const marker of BOUNDARY_MARKERS) {
        sanitized = sanitized.replace(marker.pattern, marker.replacement);
    }
    sanitized = sanitized.replace(/^(\s*)(system|assistant|user|tool|developer)\s*:\s*/gim, '$1[quoted role:$2] ');
    sanitized = sanitized.replace(/<\/?(system|assistant|user|tool|developer)\b[^>]*>/gi, (_match, role) => `[quoted tag:${role.toLowerCase()}]`);
    for (const pattern of INSTRUCTION_LIKE_FRAGMENTS) {
        sanitized = sanitized.replace(pattern, '[instruction-like text neutralized]');
    }
    return sanitized;
}
export function wrapExternalContent(options) {
