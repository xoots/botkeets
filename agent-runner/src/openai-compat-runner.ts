 ...
 */
function getContextTier(provider: string | undefined): 'full' | 'medium' | 'lean' {
  if (!provider) return 'lean';
  if (provider.startsWith('claude') || provider.includes('gpt-4o')) return 'full';
  if (provider.includes('deepseek') || provider.includes('70b') || provider.includes('llama-3')) return 'medium';
  return 'lean';
}

// ─── System prompt ────────────────────────────────────────────────────────────

function buildSystemPrompt(containerInput: ContainerInput, providerHint?: string): string {
  const parts: string[] = [];
...
function extractUrls(text: string): string[] {
  ...
  for (const raw of matches) {
    ...
    if (unique.length >= MAX_PREFETCH_URLS) break;
  }
  return unique;
}

function fetchUrlText(url: string, source = 'web.fetch'): string {
  try {
...
function augmentPromptWithPrefetchedUrls(prompt: string): string {
...
function mapPathForGroup(rawPath: string, groupFolder: string, allowProjectRoot: boolean): string {
  const trimmed = rawPath.trim();
  if (!trimmed) return trimmed;
  if (!path.isAbsolute(trimmed)) {
    return path.resolve(allowProjectRoot ? '/workspace/project' : '/workspace/group', trimmed);
  }
  if (trimmed.startsWith('/workspace/group/')) return path.normalize(trimmed);
  if (trimmed.startsWith('/workspace/project/')) return path.normalize(trimmed);
...
*/
function lintFile(filePath: string): string | null {
...
function getHistoryPath(historyDir: string, sessionId?: string): string {
  const filename = sessionId ? `history-${sessionId}.json` : 'history.json';
  return `/workspace/group/${historyDir}/${filename}`;
}

function getSessionPromptPath(historyDir: string, sessionId: string): string {
  return `/workspace/group/${historyDir}/system-prompt-${sessionId}.txt`;
}

function getCachedSessionPrompt(historyDir: string, sessionId: string): string | undefined {
...
function setCachedSessionPrompt(historyDir: string, sessionId: string, systemPrompt: string): void {
...
function assistantMessageHasToolCalls(
...
function cleanHistory(
  ...
  while (startIdx < nonSystem.length) {
    ...
    if (m.role === 'assistant' && !assistantMessageHasToolCalls(m)) break;
    startIdx++;
  }
...
function loadHistory(
  ...
  try {
    ...
    if (fs.existsSync(p)) {
      const data: SessionHistory = JSON.parse(fs.readFileSync(p, 'utf-8'));
...
*/
const TOOL_DEFINITIONS: Record<string, ChatCompletionTool> = {
bash: {
type: 'function',
function: {
name: 'bash',
...
*/
function getAvailableToolDefinitions(mode: string | undefined): Record<string, ChatCompletionTool> {
  const allowedNames = new Set(filterToolsForPolicy(Object.keys(TOOL_DEFINITIONS), mode));
  return Object.fromEntries(
    Object.entries(TOOL_DEFINITIONS).filter(([toolName]) => allowedNames.has(toolName)),
  );
}

function buildToolStubs(mode: string | undefined): ChatCompletionTool[] {
  ...
  try {
    ...
    switch (name) {
      case 'bash': {
        ...
        for (const key of secretKeys) delete env[key];
      ...
      case 'glob': {
        ...
        try {
          const out = execSync(
            `find "${searchDir}" -name "${namePattern}" -type f 2>/dev/null | head -100`,
      ...
      case 'spawn_subagent': {
        if (depth >= MAX_SWARM_DEPTH) return 'Error: Max swarm depth reached.';
        const subPrompt = String(args.prompt ?? '');
        const { result } = await runQuery(
          client,
          process.env[modelEnvVar] || model,
          systemPrompt,
          [],
          subPrompt,
          containerInput,
      ...
      case 'get_tool_definition': {
        ...
        if (!def) return `Unknown tool: ${toolName}. Available: ${Object.keys(availableDefinitions).join(', ')}`;
  ...
  for (let i = 0; i < MAX_TOOL_ITERATIONS; i++) {
    ...
    if (response.usage) {
      ...
      if (usage.totalTokens > USAGE_WARNING_THRESHOLD && !usage.warningSent) {
        ...
        executeTool(
          'send_message',
          { text: `⚠️ WARNING: Cumulative token usage in this session has exceeded ${USAGE_WARNING_THRESHOLD.toLocaleString()}. Currently at ${usage.totalTokens.toLocaleString()}.` },
          containerInput,
          secretKeys,
          client,
          usage,
...
export async function runOpenAICompatAgent(config: RunnerConfig): Promise<void> {
  ...
  const usage: UsageTracker = { totalTokens: 0, promptTokens: 0, completionTokens: 0, warningSent: false };
  ...
  try {
    ...
    while (true) {
      if (hasPrompt) {
        ...
        const { result, updatedHistory } = await runQuery(
          client, model, systemPrompt, history, prompt, containerInput, secretKeys, log, usage, 0, config.modelEnvVar
        );
        ...
        if (containerInput.isScheduledTask) break;
      }
