...
function resolveWorkspaceDir(containerInput: ContainerInput): string {
  return containerInput.workspaceDir || '/workspace/group';
}

function resolveIpcInputDir(containerInput: ContainerInput): string {
  return path.join(containerInput.ipcDir || '/workspace/ipc', 'input');
}

function resolveIpcCloseSentinel(containerInput: ContainerInput): string {
  return path.join(resolveIpcInputDir(containerInput), '_close');
}

function resolveConversationsDir(containerInput: ContainerInput): string {
  return path.join(resolveWorkspaceDir(containerInput), 'conversations');
}

function resolveGlobalClaudeMdPath(containerInput: ContainerInput): string {
  return containerInput.globalClaudeMdPath || '/workspace/global/CLAUDE.md';
}

function resolveAdditionalDirectories(containerInput: ContainerInput): string[] {
 ...
 */
function createPreCompactHook(containerInput: ContainerInput): HookCallback {
  return async (
 ...
 */
function shouldClose(containerInput: ContainerInput): boolean {
 ...
 */
function drainIpcInput(containerInput: ContainerInput): string[] {
 ...
 */
function waitForIpcMessage(containerInput: ContainerInput): Promise<string | null> {
...
async function main(): Promise<void> {
  ...
  try { fs.unlinkSync(resolveIpcCloseSentinel(containerInput)); } catch { /* ignore */ }
