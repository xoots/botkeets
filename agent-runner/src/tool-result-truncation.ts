...
const DEFAULT_POLICY: TruncationPolicy = {
  toolClass: 'generic',
  maxChars: 6_000,
  strategy: 'head_tail',
  headChars: 3_500,
...
const TOOL_POLICIES: Record<string, TruncationPolicy> = {
  bash: {
...
export function getToolResultTruncationPolicy(toolName: string): TruncationPolicy {
  return TOOL_POLICIES[toolName] ?? DEFAULT_POLICY;
}

export function truncateToolResult(toolName: string, rawContent: string): TruncatedToolResult {
